...
def calculate_perplexity(text):
    ...
    non_pad_indices = input_ids != tokenizer.pad_token_id
    input_ids = input_ids[non_pad_indices]
    logits = logits[non_pad_indices]

    # Calculate perplexity
    perplexity = torch.exp(torch.nn.functional.cross_entropy(logits.view(-1, logits.size(-1)), input_ids.view(-1)))
    return perplexity.item() if perplexity.item() != float('inf') else float('inf')


def calculate_burstiness(text, tokenizer):
    # Tokenize the text using the GPT-2 tokenizer
    encoded_input = tokenizer.encode(text, add_special_tokens=False)
    tokens = [tokenizer.decode(token_id) for token_id in encoded_input]

    # Remove special tokens and any empty strings
    tokens = [token for token in tokens if token.strip()]

    # Further processing if needed (e.g., removing stopwords, punctuation, etc.)
